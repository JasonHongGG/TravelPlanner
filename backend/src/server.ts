...
app.use((req, res, next) => {
    console.log(`[${new Date().toISOString()}] ${req.method} ${req.url} - Action: ${req.body.action}`);
    next();
});
...
app.post('/generate', async (req, res) => {
    try {
        const { userId, action, description, tripInput, location, interests, category, excludeNames, language, tripData, modificationContext } = req.body;
        ...
        // 2. Transact
        if (userId) {
            if (!apiSecret) return res.status(401).json({ error: "Missing API Secret." });
        ...
        if (action === 'GET_RECOMMENDATIONS') {
            const results = await provider.getRecommendations(location, interests, category, excludeNames, userId, apiSecret, language);
            ...
            const feasibility = await provider.checkFeasibility(tripData, modificationContext, userId, apiSecret, language);
            ...
            const trip = await provider.generateTrip(tripInput, userId, apiSecret);
...
app.post('/stream-update', async (req, res) => {
    try {
        const { userId, action, description, currentData, history, language, tripInput, dayIndex, newMustVisit, newAvoid, keepExisting, removeExisting } = req.body;
        const apiSecret = req.headers['x-api-secret'] as string;
        const provider = BackendAIService.getProvider();
        ...
        if (userId) {
            ...
            if (!apiSecret) return res.status(401).json({ error: "Missing API Secret." });
            const success = await deductPoints(userId, cost, description || action, apiSecret);
        ...
        if (action === 'CHAT_UPDATE') {
            await provider.updateTrip(currentData, history, onThought, userId, apiSecret, language);
            ...
            try {
                ...
                const tripData = await provider.generateTrip(tripInput, userId, apiSecret);
